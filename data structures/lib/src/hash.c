...
h_insert(HashTable *hash_table, char *str) {
    unsigned short int ind = hash_table->hash(str);
    List *list = hash_table->arr[ind];
    if (list == NULL) {
        //hash_table->arr[ind] = (List*)calloc(1, sizeof(List));
        hash_table->arr[ind] = hl_push_back(hash_table->arr[ind], str);
    } else {
        List *current = list;
        while (current->next != NULL) {
        ...
        current->next->data = strdup(str);
    }
}

void
h_erase(HashTable *hash_table, char *str) {
    unsigned int ind = hash_table->hash(str);
    List *list = hash_table->arr[ind];
    if (list == NULL) {
